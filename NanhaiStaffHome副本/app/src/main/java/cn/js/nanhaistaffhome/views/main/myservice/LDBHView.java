 ...
 * 劳动保护
 */
public class LDBHView extends RefreshListView{

    private List<News> newsList = new ArrayList<>();
    private NewsAdapter adapter;

    public LDBHView(Context context){
        super(context);
        init(context);
    }

    public LDBHView(Context context,AttributeSet attrs){
        super(context, attrs);
        init(context);
    }
    ...
    private void init(Context context){
        ...
        setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ActivityManager.toNewsDetailActivity(getContext(), newsList.get(i - 1).getContentId(),"劳动保护");
            }
        });
    ...
    private void getServiceList(String time,final int type){
        HttpClient.getNewsByTime(469, 10, time, type, new OnHttpRequestListener() {
            @Override
