...
public class BitmapUtil {
	...
	public static Bitmap getimage(String srcPath,CompressFormat format) {
	...
	/**
	 * 图片按比例大小压缩方法
	 * @param level 压缩系数(1~100)
	 * */
	public static Bitmap comp(Bitmap image,int level,CompressFormat format) {
		
		ByteArrayOutputStream baos = new ByteArrayOutputStream();		
		image.compress(format, 100, baos);
		if( baos.toByteArray().length / 1024>1024) {//判断如果图片大于1M,进行压缩避免在生成图片（BitmapFactory.decodeStream）时溢出	
			baos.reset();//重置baos即清空baos
			image.compress(CompressFormat.JPEG, level, baos);//这里压缩50%，把压缩后的数据存放到baos中
		}
